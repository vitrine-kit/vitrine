...
- 0fc03c4: Enrich the Payload zero-config demo seed and storefront examples so a fresh `vitrine init` shop is immediately browseable: realistic prices, multi-size/color variants, gallery images, category copy, a home hero with guided CTAs, variant pickers on Add to cart, and a cart empty-state that points at Classic T-Shirt.

## 0.4.2

### Patch Changes

- ebf83ec: Scaffolded Next configs now set webpack `resolve.extensionAlias` so TypeScript ESM-style relative imports (`.js` → `.ts`/`.tsx`) resolve under `next dev`. Without this, client projects fail to compile feature components like `ProductCard.js`.
...
- 158abd9: `vitrine init` / `add` now work immediately after a global install — the CLI ships the
